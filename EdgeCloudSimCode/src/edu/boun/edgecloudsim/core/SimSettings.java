...
public class SimSettings {
	...
	public static final int GENERIC_EDGE_DEVICE_ID = 1003; //*Mine* edge device (wifi access point)
    ...
    private String[] taskNames = null;//*Mine* contains the name of application {AUGMENTED_REALITY, HEALTH_APP, HEAVY_COMP_APP, INFOTAINMENT_APP}
	...
	public boolean initialize(String propertiesFile, String edgeDevicesFile, String applicationsFile){
		...
		try {
			...
			WAN_PROPOGATION_DELAY = Double.parseDouble(prop.getProperty("wan_propogation_delay"));//*Mine* seconds
			LAN_INTERNAL_DELAY = Double.parseDouble(prop.getProperty("lan_internal_delay"));//*Mine* seconds
			BANDWITH_WLAN = 1000 * Integer.parseInt(prop.getProperty("wlan_bandwidth"));//*Mine* Kbps
			BANDWITH_WAN = 1000 * Integer.parseInt(prop.getProperty("wan_bandwidth"));//*Mine* Kbps
			BANDWITH_GSM =  1000 * Integer.parseInt(prop.getProperty("gsm_bandwidth"));//*Mine* Kbps
			...
			//mean waiting time (minute) ==> Values are {480, 300, 120}
