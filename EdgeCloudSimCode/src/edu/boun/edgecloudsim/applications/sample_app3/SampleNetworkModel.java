...
public class SampleNetworkModel extends NetworkModel {
	...
	public void initialize() {
		wlanClients = new int[SimSettings.getInstance()
				.getNumOfEdgeDatacenters()]; // we have one access point for
												// each datacenter
	}
	...
	public double getUploadDelay(int sourceDeviceId, int destDeviceId, Task task) {
		...
		if (destDeviceId == SimSettings.GENERIC_EDGE_DEVICE_ID) {
			delay = getWlanUploadDelay(task.getSubmittedLocation(),
					task.getCloudletFileSize());
		} else {
			SimLogger
					.printLine("Error - unknown device id in getUploadDelay(). Terminating simulation...");
			System.exit(0);
		}
		return delay;
	}
	...
	public double getDownloadDelay(int sourceDeviceId, int destDeviceId,
		...
		Location accessPointLocation = SimManager.getInstance()
				.getMobilityModel().getLocation(destDeviceId, CloudSim.clock());

		// edge device (wifi access point) to mobile device
		if (sourceDeviceId == SimSettings.GENERIC_EDGE_DEVICE_ID) {
			delay = getWlanDownloadDelay(accessPointLocation,
					task.getCloudletOutputSize());
		} else {
			SimLogger
					.printLine("Error - unknown device id in getDownloadDelay(). Terminating simulation...");
			System.exit(0);
		}
	...
	public void uploadStarted(Location accessPointLocation, int destDeviceId) {
		if (destDeviceId == SimSettings.GENERIC_EDGE_DEVICE_ID) {
			...
			SimLogger
					.printLine("Error - unknown device id in uploadStarted(). Terminating simulation...");
			System.exit(0);
		}
	}
	...
	public void uploadFinished(Location accessPointLocation, int destDeviceId) {
		if (destDeviceId == SimSettings.GENERIC_EDGE_DEVICE_ID) {
			...
			SimLogger
					.printLine("Error - unknown device id in uploadFinished(). Terminating simulation...");
			System.exit(0);
		}
	}
	...
	public void downloadStarted(Location accessPointLocation, int sourceDeviceId) {
		if (sourceDeviceId == SimSettings.GENERIC_EDGE_DEVICE_ID) {
			...
			SimLogger
					.printLine("Error - unknown device id in downloadStarted(). Terminating simulation...");
			System.exit(0);
		}
	}

	@Override
	public void downloadFinished(Location accessPointLocation,
			int sourceDeviceId) {
		if (sourceDeviceId == SimSettings.GENERIC_EDGE_DEVICE_ID) {
			wlanClients[accessPointLocation.getServingWlanId()]--;
		} else {
			SimLogger
					.printLine("Error - unknown device id in downloadFinished(). Terminating simulation...");
			System.exit(0);
		}
	}
	...
	private double getWlanDownloadDelay(Location accessPointLocation, double dataSize) {
		...
		if (numOfWlanUser < experimentalWlanDelay.length)
			result = taskSizeInKb /* Kb */
					/ (experimentalWlanDelay[numOfWlanUser] * (double) 3) /* Kbps */; // 802.11ac is around 3 times faster than 802.11n

		// System.out.println("--> " + numOfWlanUser + " user, " + taskSizeInKb + " KB, " +result + " sec");
		// *Mine* This method is added by me to print the delay (upload and download into file)
		printdelay(numOfWlanUser, taskSizeInKb, result);
		return result;
	}

	// *Mine*  This method is added by me to Print the delay into the file (upload and download delay)
	private void printdelay(int numOfWlanUser, double taskSizeInKb,
			double result) {
		String FileLocation = "E:\\HIT\\OneDrive - Computer and Information Technology (Menofia University)\\My Papers\\Git Code\\Other codes\\Cloud_EdgeSim_Code\\cloudsim-3.0.3\\scripts\\sample_app3\\config\\Files\\NetworkDelay.txt";
		BufferedWriter writer = null;
		try {
			writer = new BufferedWriter(new FileWriter(FileLocation, true));
			writer.append("--> " + numOfWlanUser + " user, " + taskSizeInKb
					+ " KB, " + result + " sec");
			writer.newLine();
	...
	private double getWlanUploadDelay(Location accessPointLocation,
			double dataSize) {
