...
public class TestCloudServerManager {
	...
	public static void main (String[] args){
		...
		int index=SS.CLOUD_DATACENTER_ID;
		...
		System.out.println("Cloud DataCenter:"+index+" arch: "+arch+" os: "+os+" vmm: "+vmm+" costPerBw: "+costPerBw+" costPerSec: "+costPerSec +" costPerMem: "+costPerMem+" costPerStorage: "+costPerStorage);

		List<Host> hostList1=null;
		...
		// Here are the steps needed to create a PowerDatacenter:
				...
				for (int i = 0; i < SS.getNumOfCoudHost(); i++) {
					int numOfVMPerHost = SS.getNumOfCloudVMsPerHost();
					int numOfCores = SS.getCoreForCloudVM() * numOfVMPerHost;
					double mips = SS.getMipsForCloudVM() * numOfVMPerHost;
					int ram = SS.getRamForCloudVM() * numOfVMPerHost;
					long storage = SS.getStorageForCloudVM() * numOfVMPerHost;
					long bandwidth = 0;
					...
					System.out.println("Cloud Host:"+i+" numOfVMPerHost: "+numOfVMPerHost+" numofCores: "+numOfCores+" mips: "+mips+" ram: "+ram+" storage: "+storage+" bandwidth: "+bandwidth);
		...
		VmAllocationPolicy vm_policy = new CloudVmAllocationPolicy_Custom(hostList,index);
		try {
